...
export default function Categories() {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="grid gap-3 rounded-xl bg-white p-5 shadow-sm sm:grid-cols-4">
        <input
          type="text"
          required
          placeholder="Nome"
          ...
          <ul className="divide-y divide-slate-100">
            {categories.map((category) => (
              <li key={category.id} className="flex items-center justify-between py-3 text-sm">
                ...
                <button onClick={() => handleDelete(category.id)} className="text-slate-400 hover:text-red-600">
                  Remover
                </button>
              </li>
            ))}
